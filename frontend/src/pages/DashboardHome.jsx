...
export default function DashboardHome() {
    ...
    try {
      if (isSuperAdmin) {
        ...
        if (role === ROLES.ORG_ADMIN) {
          promises.push(orgApi.getJoinRequests(user?.orgId).catch(() => ({ data: [] })))
        }
        
        const results = await Promise.all(promises)
        setProjects(Array.isArray(results[0].data) ? results[0].data : [])
        setTeamsCount(Array.isArray(results[1].data) ? results[1].data.length : 0)
        setMembersCount(Array.isArray(results[2].data) ? results[2].data.length : 0)
  ...
  async function handleDeleteOrg(org) {
    if (!confirm(`Are you sure you want to permanently delete the organization "${org.name}"? This action cannot be undone.`)) return
    setError('')
        ...
        <>
          ...
          {role === ROLES.ORG_ADMIN && (
            <div className="dh-card" style={{ marginTop: 20 }}>
              <div className="dh-card-header">
                <h3 className="dh-card-title">Pending Join Requests</h3>
                <span style={{ fontSize: 13, color: '#64748b' }}>{joinRequests.length} total</span>
              </div>
              {actionError && <p className="wk-alert wk-alert-error" style={{ marginBottom: 10 }}>{actionError}</p>}
              {loading ? (
                <p className="dh-empty">Loading join requests…</p>
              ) : joinRequests.length === 0 ? (
                <p className="dh-empty">No pending join requests.</p>
              ) : (
                <table className="wk-table" style={{ marginTop: 10 }}>
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    {joinRequests.map(req => (
                      <tr key={req.id}>
                        <td style={{ fontWeight: 600 }}>{req.fullName || 'User'}</td>
                        <td>{req.email}</td>
                        <td>{req.role}</td>
                        <td>
                          <div style={{ display: 'flex', gap: 10 }}>
